...
const Report = () => {
  ...
  const columnSample = [
    ...
    },
    {
      ...
      header: "Evidence",
      cell: (info) => (
        <img className="w-20 h-20 rounded-full" src={getRealFileUrl(info.getValue())} />
      ),
    },
    {
...
function EditTagForm({ info, tagsData, setTagsData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await customFetch.put('/tag/update-tag/' + info.cell.row.original._id, formDataForUpload);
      ...
      if (response.data?.status !== 'error') {
        ...
        let updatedTags = tagsData.map((tag) => {
          if (tag._id === info.cell.row.original._id) {
            let updatedTag = { ...tag, action: formData.action, text: formData.text }
            return updatedTag
          }
          return tag
        })
      ...
      console.log(error)
      const errorMessage = error?.response?.data?.msg || 'Error updating Tag';
