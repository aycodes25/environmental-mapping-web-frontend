...
export function SceneComponent({
  ...
  function delayCreateScene(engine, baseUrl, filenameWithExtension) {
    ...
    scene.onBeforeRenderObservable.add(() => {
      ...
      arrow.position = scene.activeCamera.position.add(forward.scale(distance));
...
function returnToNormalView() {
  ...
  window.scene.activeCamera.position = saveCameraPositionAndDirection.position.clone()
  window.scene.activeCamera.direction = saveCameraPositionAndDirection.direction.clone()
  window.scene.activeCamera.rotation = saveCameraPositionAndDirection.rotation.clone()
...
function populateTags(scene, position, modelId) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    let { cameraPosition, cameraRotation, cameraDirection } = getCamDetailsLookingDown(randomPosition)
...
async function persistAutoGeneratedTags() {
  await Promise.all(globalTagDescs.map(async (tagDesc) => {
...
function getRandomAction(type) {
  if (type === "sampling") {
    const randomIndex = Math.floor(Math.random() * samplingRemedialActions.length);
    return samplingRemedialActions[randomIndex];
  } else if (type === "incident") {
    const randomIndex = Math.floor(Math.random() * safetyRemedialActions.length);
