...
export default function ObjectGroups({ objectGroups, setObjectGroups, modelId, fetchObjectGroups }) {
    ...
    const handleDeleteGroup = async (group) => {
        try {
            const response = await customFetch.delete(`/model/${modelId}/object-group/${group._id}`);
    ...
    const handleCreateGroup = async (event) => {
        ...
        try {
            const response = await customFetch.post(`/model/${modelId}/object-group`, {
                name: groupName,
                ...groupData,
            });
    ...
    return (
        <div className="p-4 space-y-4">
            ...
            <div className="flex flex-col gap-4">
                ...
                {showCreateForm ? (
                    <form onSubmit={handleCreateGroup} className="flex flex-col gap-4">
                        ...
                        <datalist id="group-names">
                            {[...new Set(objectGroups.map(group => group?.name))].filter(name => name).map((name) => (
                ...
                {/* Create Group Button */}
                {!showCreateForm ? <div className="w-full">
                    <button
